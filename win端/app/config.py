...
def load_settings(env_file: str | Path | None = None) -> Settings:
    ...
    profile_raw = _optional_env("DOUYIN_PROFILE_DIR")
    return Settings(
        ...
        trace=_parse_bool(os.getenv("TRACE", "false"), "TRACE"),
        dingtalk_webhook=dingtalk_webhook,
        dingtalk_secret=dingtalk_secret,
        profile_dir=Path(profile_raw).expanduser() if profile_raw else None,
    )
...
def load_task(settings: Settings) -> TaskConfig:
    ...
    task = TaskConfig(
        ...
        daily_max_sends=_non_negative_int(raw.get("daily_max_sends", 20), "daily_max_sends"),
        skip_if_today_outgoing=bool(raw.get("skip_if_today_outgoing", True)),
    )
