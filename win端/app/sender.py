...
from app.douyin import DouyinChat, PageOperationError, first_visible, type_text_like_human
...
async def send_text(chat: DouyinChat, content: str) -> None:
    ...
    page = editor.page
    await editor.click()
    await type_text_like_human(page, content)
    try:
    ...
    await page.wait_for_timeout(random.randint(250, 600))  # 发送前拟人停顿
...
async def send_image(page: Page, image_path: str) -> None:
    ...
    await page.wait_for_timeout(random.randint(1_200, 2_200))  # 图片上传与渲染的拟人等待
