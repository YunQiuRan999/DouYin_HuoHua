...
from app.douyin import DouyinChat, has_today_outgoing_message
        ...
        async with open_douyin(settings) as session:
            ...
            if fatal_error is None:
                ...
                for index, target in enumerate(task.targets):
                    ...
                    try:
                        if task.daily_max_sends > 0 and total_sent >= task.daily_max_sends:
                            LOGGER.info("已达到今日发送上限 %d 条，停止继续发送", task.daily_max_sends)
                            break
                        LOGGER.info("处理好友: %s", alias)
                        await chat.open_target(target.name, retries=task.target_open_retries)
                        if not dry_run:
                            if task.skip_if_today_outgoing and await has_today_outgoing_message(page):
                                LOGGER.info("检测到 %s 今天已发送过消息，跳过自动发送（真人已续火花）", alias)
                                results.append(TargetResult(target=target.name, status="skipped_today", sent=0, target_alias=alias))
                                continue
                    ...
                    if index < len(task.targets) - 1 and not dry_run:
                        await asyncio.sleep(_human_pause())
    ...
    if reset or not LOGGER.handlers:
        for handler in list(LOGGER.handlers):
            if getattr(handler, "_keep", False):
                continue  # 服务端附加的日志广播 handler 等：reset 时保留
        ...
        # 打包为窗口程序（PyInstaller windowed）后 sys.stderr 为 None，跳过控制台输出。
...
async def _screenshot(page, artifacts_dir: Path, label: str) -> Path | None:
    ...
    try:
        # full_page 在长会话页可能很慢，加超时保护避免失败路径拖慢整个任务
        await asyncio.wait_for(page.screenshot(path=path, full_page=True), timeout=15)
        return path
    except Exception:
...
def _write_results(
    ...
    _atomic_write_json(artifacts_dir / "result.json", payload)


def _atomic_write_json(path: Path, data: dict) -> None:
    """原子写入 JSON 结果文件，与 config.json 的 write_json 保持一致。

    运行中途进程崩溃时，裸 write_text 会留下半写/损坏的 result.json，
    导致 runner._parse_result 读取出错而把一次基本成功的运行误判为 failed。
    """
...
def _human_pause() -> float:
    """好友之间的拟人化停顿（秒）：常规 30-90 秒，小概率出现 2-5 分钟长停顿。

    真人从一个好友切换到下一个（搜索→点开→发送→返回列表）通常需要 1-3 分钟；
    均匀且短暂的固定间隔是机器行为的显著特征。引入随机长停顿更接近真人，
    降低连续操作多个好友时的风控识别概率。
    """
